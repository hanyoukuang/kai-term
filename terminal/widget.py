    ...
    def _poll_updates(self) -> None:
        try:
            ...
            try:
                ...
                _log.warning("PTY session ended (detected in poll)")
                self._session_ended = True
                self.process_exited.emit(-1)
                return
            if has_updates:
                ...
                if self._stale_polls >= 60:
                    try:
                        ...
                        _log.warning("PTY session ended (during stale flush)")
                        self._session_ended = True
                        self.process_exited.emit(-1)
                        return
                    except Exception:
    ...
    def _paste_text(self, text: str) -> None:
        try:
            if self._term.bracketed_paste():
                self._write_to_pty("\x1b[200~" + text + "\x1b[201~")
            else:
                self._write_to_pty(text)
        except Exception:
            self._write_to_pty(text)
