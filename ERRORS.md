...
| 8 | 06-04 | Phase 6 | ~~鼠标拖拽选择文字后高亮不消失~~（非 Bug，见下方） + 无复制通知机制 | ~~高亮保留是标准行为~~ 真正问题：无 `selection_copied` Signal 让宿主感知复制事件 | 添加 `selection_copied` Signal + `clear_selection()` 公开方法 |
...
## 错误 #8 详细分析：~~选择高亮不消失~~ + 无复制通知

### 纠正：高亮不消失是标准行为，不是 Bug

> 标准终端模拟器（xterm.js、Alacritty、iTerm2、Windows Terminal）在鼠标拖拽释放后**不会主动清除高亮**。
> 高亮保留作为视觉参考，仅在以下情况清除：
> - 用户下次点击（`mousePressEvent` 已实现 `_clear_selection()`）
> - 键盘输入（`keyPressEvent` 已实现）
> - 宿主应用主动调用 `clear_selection()`

### 真正的问题：缺少通知机制

`_copy_selection()` 只写系统剪贴板，宿主应用无法感知复制事件。

### 修复

1. 添加 `selection_copied = Signal(str)` — 同进程通知
2. 添加 `clear_selection()` 公开方法 — 宿主可主动清除高亮
3. `mouseReleaseEvent`：拖拽释放后发信号，**不主动清高亮**
4. `mousePressEvent`：新点击时清旧高亮（已有）
